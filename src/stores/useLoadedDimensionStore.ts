...
export const useLoadedDimensionStore = defineStore('loaded_dimension', () => {
    ...
    function getIcon(id: Identifier, x?: number, z?: number){
        const structureId = id.toString()

        // 1. 檢查用戶自定義圖示（位置或結構類型，最高優先級）
        const structureNotesStore = useStructureNotesStore()
        const customIcon = structureNotesStore.getStructureIcon(structureId, x, z)
        if (customIcon) {
            // 本地路徑直接返回，否則使用 mcicons URL
            if (customIcon.startsWith('/')) {
                return customIcon
            }
            return `https://raw.githubusercontent.com/jacobsjo/mcicons/icons/item/${customIcon}.png`
        }

        // 2. 使用預設映射（本地 webp 圖示）
        const defaultIcon = DEFAULT_STRUCTURE_ICONS[structureId]
        if (defaultIcon) {
            // 本地路徑直接返回
            if (defaultIcon.startsWith('/')) {
                return defaultIcon
            }
            return `https://raw.githubusercontent.com/jacobsjo/mcicons/icons/item/${defaultIcon}.png`
        }

        // 3. 檢查 datapack 定義的圖示（作為備用）
        const datapackIcon = loaded_dimension.structure_icons?.get(structureId)
        if (datapackIcon) {
            return `https://raw.githubusercontent.com/jacobsjo/mcicons/icons/item/${datapackIcon.path}.png`
        }

        // 4. 回退到隨機圖示
        const randomItem = VANILLA_ITEMS[Math.abs(hashCode(structureId)) % VANILLA_ITEMS.length]
        return `https://raw.githubusercontent.com/jacobsjo/mcicons/icons/item/${randomItem}.png`
    }
